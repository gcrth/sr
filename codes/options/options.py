...
def check_resume(opt, resume_iter):
    ...
    if opt['path']['resume_state']:
        ...
        opt['path']['pretrain_model_G'] = osp.join(osp.split(opt['path']['resume_state'])[0],'..','models',
                                                    # opt['path']['models'],
                                                   '{}_G.pth'.format(resume_iter))
        logger.info('Set [pretrain_model_G] to ' + opt['path']['pretrain_model_G'])
        if 'gan' in opt['model']:
            opt['path']['pretrain_model_D'] = osp.join(osp.split(opt['path']['resume_state'])[0],'..','models',
