...
def profile(model, inputs, custom_ops=None, verbose=True):
    ...
    def add_hooks(m):
        ...
        m.register_buffer('total_ops', torch.zeros(1).to(inputs[0].device))
        m.register_buffer('total_params', torch.zeros(1).to(inputs[0].device))

        for p in m.parameters():
            m.total_params += torch.Tensor([p.numel()]).to(inputs[0].device)
