...
def zero_ops(m, x, y):
    m.total_ops += torch.Tensor([int(0)]).to(m.total_ops.device)
...
def count_convNd_ver2(m: _ConvNd, x: (torch.Tensor,), y: torch.Tensor):
    ...
    m.total_ops += torch.Tensor([int(output_size * kernel_ops)]).to(m.total_ops.device)
...
def count_relu(m, x, y):
    ...
    m.total_ops += torch.Tensor([int(nelements)]).to(m.total_ops.device)
...
def count_adap_avgpool(m, x, y):
    kernel = torch.Tensor([*(x[0].shape[2:])]).to(m.total_ops.device) // torch.Tensor(list((m.output_size,))).squeeze().to(m.total_ops.device)
